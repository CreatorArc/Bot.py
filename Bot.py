...
# 6. Admin Approval / Rejection Trigger (FIXED ERROR HANDLING)
...
def handle_admin_action(call):
    ...
    if action == "app":
        # 1. User ko Link bhejna (with joining button & fallback)
        try:
            join_btn = types.InlineKeyboardMarkup()
            join_btn.add(types.InlineKeyboardButton("🚀 Join Channel / Group", url=GROUP_INVITE_LINK))
